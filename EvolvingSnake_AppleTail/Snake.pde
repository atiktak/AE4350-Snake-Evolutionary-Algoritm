class Snake {
  ...
  void look() {
    ...
    for (int i = 0; i < lvl; i++) {
      ...
      if (direction > 1) {
        if (snakeX[i]==x) {
          if (Dtail[0] > (snakeX[i] - x + width) % width) {
            Dtail[0] = (snakeX[i] - x + width) % width;
          }
        }
      } else {
        if (snakeY[i]==y) {
          if (Dtail[1] > (snakeY[i] - y + height) % height) {
            Dtail[1] = (snakeY[i] - y + height) % height;
          }
        }
      }
      ...
      // Forward vision
      if (direction == 0) {
        if (Dtail[0] > (x - snakeX[i] + width) % width) {
          Dtail[0] = (x - snakeX[i] + width) % width;
        }
      } else if (direction == 1) {
        if (Dtail[0] > (snakeX[i] - x + width) % width) {
          Dtail[0] = (snakeX[i] - x + width) % width;
        ...
        if (Dtail[1] > (y - snakeY[i] + height) % height) {
          Dtail[1] = (y - snakeY[i] + height) % height;
        }
      } else {
        if (Dtail[1] > (snakeY[i] - y + height) % height) {
          Dtail[1] = (snakeY[i] - y + height) % height;
  ...
  void decide() {
    float[] inputs = concat(Dapple, Dtail);
